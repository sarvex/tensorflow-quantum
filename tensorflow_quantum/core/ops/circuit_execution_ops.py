...
def _check_quantum_concurrent(quantum_concurrent):
    if not isinstance(quantum_concurrent, bool):
        raise TypeError(
            f"quantum_concurrent must be type bool. Given: {str(type(quantum_concurrent))}"
        )
...
def get_expectation_op(
    ...
    raise TypeError(
        f"Backend {backend} is invalid. Expected a cirq.sim.simulator.SimulatesExpectationValues or None."
    )
...
def get_state_op(
    ...
    raise TypeError(
        f"Backend {backend} is invalid. Expected a Cirq.SimulatesFinalState or None."
    )
